...
<div x-data="{ showPassword: false }" class="relative mt-1">
    <input
        ...
        @required($required)
        @autofocus($autofocus)
        {{ $attributes->merge(['class' => 'block w-full rounded-md border-slate-300 pr-11 shadow-sm focus:border-blue-500 focus:ring-blue-500']) }}
    >
    <button
        type="button"
        ...
        class="absolute inset-y-0 right-0 inline-flex w-11 items-center justify-center rounded-r-md text-slate-400 transition hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500 dark:text-slate-400 dark:hover:text-blue-200"
    >
