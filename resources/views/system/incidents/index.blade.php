<x-app-layout>
    ...
    <div class="py-5 sm:py-8">
        <div class="mx-auto max-w-7xl space-y-5 px-4 sm:px-6 lg:px-8">
            ...
            <form method="GET" action="{{ route('incidents.index') }}" class="grid gap-4 rounded-lg border border-blue-100 bg-white p-4 shadow-sm md:grid-cols-4">
                <div>
            ...
            <div class="space-y-4">
                @forelse ($incidents as $incident)
                    ...
                    @endphp
                    <article class="rounded-lg border border-blue-100 bg-white p-4 shadow-sm sm:p-5">
                        <div class="grid gap-5 lg:grid-cols-[1fr_360px]">
                            <div>
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <h3 class="text-lg font-semibold text-blue-950">{{ $incident->category }}</h3>
                                        <span class="rounded-full px-2.5 py-1 text-xs font-semibold ring-1 {{ $incident->priority === 'critical' || $incident->priority === 'high' ? 'bg-red-50 text-red-700 ring-red-200' : 'bg-blue-50 text-blue-700 ring-blue-200' }}">
                                            {{ ucfirst($incident->priority) }}
                                        </span>
                                        <span class="rounded-full bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-700 ring-1 ring-slate-200">
                                            {{ str($incident->status)->replace('_', ' ')->title() }}
                                        </span>
                                    </div>
                                    <a href="{{ route('incidents.pdf', $incident) }}" class="inline-flex shrink-0 items-center justify-center rounded-md border border-blue-200 px-3 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                                        Download PDF
                                    </a>
                                </div>
                                ...
                                @if ($imagePaths->isNotEmpty())
                                    <div class="mt-4 grid gap-2" style="grid-template-columns: repeat(auto-fit, minmax(9rem, 1fr));">
                                        @foreach ($imagePaths as $imagePath)
                                            <a href="{{ asset('storage/'.$imagePath) }}" target="_blank" class="block overflow-hidden rounded-lg border border-blue-100 bg-blue-50">
                            ...
                            <form method="POST" action="{{ route('incidents.update', $incident) }}" class="rounded-lg border border-blue-100 p-4">
                                @csrf
                                @method('PATCH')
                                <div>
                ...
                @empty
                    <div class="rounded-lg border border-blue-100 bg-white px-5 py-8 text-center text-slate-500 shadow-sm">No incident reports found.</div>
                @endforelse
            </div>
