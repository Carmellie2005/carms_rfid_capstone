            ...
            @foreach ($logs as $log)
                ...
                <section x-show="selectedPatrolId === @js((string) $log->id)" x-cloak class="flex max-h-[88dvh] w-full max-w-5xl flex-col overflow-hidden rounded-md border border-blue-100 bg-white shadow-2xl">
                    ...
                    <div class="min-h-0 flex-1 overflow-y-auto px-5 py-4">
                        <dl class="grid gap-3 rounded-md border border-blue-100 bg-blue-50/30 p-4 text-sm sm:grid-cols-2 lg:grid-cols-5">
                            <div class="min-w-0 lg:border-r lg:border-blue-100 lg:pr-3">
                            ...
                            <div class="min-w-0 lg:border-r lg:border-blue-100 lg:pr-3">
                                ...
                                <dd class="mt-1 text-slate-700">{{ $detailScanTime?->format('M d, Y h:i A') ?? 'No scan time' }}</dd>
                            </div>
                            <div class="min-w-0 lg:border-r lg:border-blue-100 lg:pr-3">
                                <dt class="font-bold text-slate-600">RFID</dt>
                                <dd class="mt-1 truncate font-mono text-slate-700">{{ $log->rfid_uid }}</dd>
                            </div>
                            <div class="min-w-0">
                            ...
                            <div class="rounded-md border border-blue-100 bg-white p-4">
                                ...
                                @if ($log->checklistResponse)
                                    ...
                                    @if ($log->checklistResponse->remarks)
                                        <div class="mt-4 rounded-md border border-blue-100 bg-slate-50 px-3 py-2 text-sm text-slate-600">
                                        ...
                                        <div class="mt-2 grid grid-cols-3 gap-2">
                                            @foreach ($detailProofPhotos as $photo)
                                                ...
                                                <button type="button" class="group relative aspect-square overflow-hidden rounded-md border border-blue-100 bg-slate-100 shadow-sm transition hover:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2" @click="openProofPhoto(@js($photoUrl), @js($photo->item_label))" aria-label="Open {{ $photo->item_label }} proof photo">
                                                    ...
                                                    <span class="absolute inset-x-1 bottom-1 truncate rounded bg-slate-950/70 px-1.5 py-0.5 text-[0.65rem] font-semibold text-white">{{ $photo->item_label }}</span>
                                                </button>
                                            @endforeach
                                        </div>
                                    @endif
